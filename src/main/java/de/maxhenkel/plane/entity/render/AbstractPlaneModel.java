...
public abstract class AbstractPlaneModel<T extends EntityPlaneSoundBase> extends OBJRenderer<T> {
    ...
    public void render(T plane, float yaw, float partialTicks, MatrixStack matrixStack, IRenderTypeBuffer buffer, int light) {
        ...
        if (plane.hasCustomName()) {
            String name = trimName(plane.getCustomName().getFormattedText(), 0.02F, 1F);
