...
public class PlaneScreen extends ScreenBase<ContainerPlane> {
    ...
    public PlaneScreen(ContainerPlane containerCar, Inventory playerInv, Component title) {
        super(CAR_GUI_TEXTURE, containerCar, playerInv, title, 176, 222);
    ...
    protected void extractLabels(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
        ...
        guiGraphics.text(font, plane.getName().getVisualOrderText(), 7, 61, FONT_COLOR, false);
        guiGraphics.text(font, playerInv.getDisplayName().getVisualOrderText(), 8, this.imageHeight - 96 + 2, FONT_COLOR, false);

        guiGraphics.text(font, TEXT_FUEL.getVisualOrderText(), 7, 9, FONT_COLOR, false);
        guiGraphics.text(font, TEXT_DAMAGE.getVisualOrderText(), 95, 9, FONT_COLOR, false);
        guiGraphics.text(font, TEXT_ENGINE.getVisualOrderText(), 7, 35, FONT_COLOR, false);
    ...
    public void drawThrottle(GuiGraphicsExtractor guiGraphics, float percent) {
        int scaled = (int) (72F * percent);
        int i = leftPos;
        int j = topPos;
